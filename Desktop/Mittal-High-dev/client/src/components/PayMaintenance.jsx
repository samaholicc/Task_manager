...
function PayMaintenance(props) {
  ...
  const pay = async () => {
    try {
      ...
      setRows(result);
      setIsPaid(result.stat === "payé"); // Translated "paid" to "payé"
    } catch (error) {
      console.log(error);
    }
  };
  ...
  useEffect(() => {
    axios
      .post(`${process.env.REACT_APP_SERVER}/paymaintanance`, {
        ...
        status: "Payé", // Translated "Paid" to "Payé"
      })
      .then((res) => {
        // console.log(res);
      })
                  ...
                  <tr>
                    ...
                    >
                      {!isPaid ? (
                        ...
                        >
                          Payer {/* Translated "Pay" to "Payer" */}
                        </button>
                      ) : (
                        <span className="px-6 py-2 border-2 border-transparent">
                          Payé {/* Translated "Paid" to "Payé" */}
                        </span>
                      )}
                    </td>
                  </tr>
